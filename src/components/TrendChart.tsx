...
const metricConfig: Record<MetricKey, { label: string; shortLabel: string }> = {
  trueBalance: { label: 'Cash Prophet Balance', shortLabel: 'Balance' },
...
export function TrendChart({
  ...
  const { series, sortedDates, xTickMarks, yTicks, yMin, yMax, zeroY, projections, projectionBoundaryX } =
    ...
    scopeOptions.forEach((opt, scopeIdx) => {
      ...
      const dailySnaps = snaps.map((snap) => withEffectiveSnapshotMetrics(state, snap))

      for (const metric of activeMetricKeys) {
        let metricSnaps = alignSnapshotsWithBalanceLogRollup(
          state,
          viewScope,
          graphRange,
          opt.scope,
        ...
        metricSnaps = aggregateSnapshotsForGranularity(metricSnaps, historyGranularity)
        metricSnaps.forEach((s) => {
          dateSet.add(s.date)
        ...
        <div className={`trends-chart-plot${embedded ? ' trends-chart-plot--embedded' : ''}`}>
          <div className="chart-wrap">
            <svg
              ...
              role="img"
              aria-label="Cash Prophet Balance trend chart — financial position over time for your business"
          ...
          {detailSnapshot && pinned && (
            <div className="chart-tooltip">
              ...
              <p>Cash Prophet Balance: {formatCurrency(getEffectiveSnapshotMetric(state, detailSnapshot, 'trueBalance'))}</p>
