...
function loadGranularity(): HistoryGranularity {
  try {
...
export function TrueBalanceHistoryTable({
  ...
  const setHistoryGranularity = (next: HistoryGranularity) => {
    setGranularity(next)
    try {
      localStorage.setItem(GRANULARITY_STORAGE_KEY, next)
    } catch {
      /* ignore */
    }
  }
        ...
        <div className="trends-history-head">
          <div>
            ...
            <p className="muted trends-history-lead">
              Same data points as the chart — one row per day you saved balances.
            </p>
          </div>
