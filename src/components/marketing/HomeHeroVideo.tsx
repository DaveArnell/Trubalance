...
/**
 * Homepage product snapshot. Heading lives on the parent section when embedded.
 * Swaps to the walkthrough video when a homepage video URL is published.
 */
export function HomeHeroVideo({ embedded = false }: { embedded?: boolean }) {
      ...
      {url ? (
        ...
        <figure className="home-video-snapshot">
          ...
          <figcaption className="home-video-snapshot-actions">
            <CanonicalLink to="/see-how-it-works" className="btn-secondary">
              Try a live demo
            </CanonicalLink>
          </figcaption>
        </figure>
  ...
  return (
    ...
    >
      <div className="marketing-section-inner marketing-section-inner--home home-band-stack">{frame}</div>
    </section>
  )
}
